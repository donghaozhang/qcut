...
Options:
  ...
  --provider google|openai|dashscope|replicate|fal  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "replicate" && v !== "fal") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, replicate: null, fal: null };
        currentKey = "default_model";
      } else if (currentKey === "default_model" && (key === "google" || key === "openai" || key === "dashscope" || key === "replicate" || key === "fal")) {
        const cleaned = value.replace(/['"]/g, "");
...
function detectProvider(args: CliArgs): Provider {
  if (args.referenceImages.length > 0 && args.provider && args.provider !== "google" && args.provider !== "openai" && args.provider !== "replicate" && args.provider !== "fal") {
    throw new Error(
      "Reference images require a ref-capable provider. Use --provider google (Gemini multimodal), --provider openai (GPT Image edits), --provider replicate, or --provider fal (edit models)."
    );
  }
  ...
  if (args.referenceImages.length > 0) {
    ...
    throw new Error(
      "Reference images require Google, OpenAI, fal.ai or Replicate. Set GOOGLE_API_KEY/GEMINI_API_KEY, OPENAI_API_KEY, FAL_KEY, or REPLICATE_API_TOKEN, or remove --ref."
    );
  }

  const available = [hasGoogle && "google", hasOpenai && "openai", hasFal && "fal", hasDashscope && "dashscope", hasReplicate && "replicate"].filter(Boolean) as Provider[];

  if (available.length === 1) return available[0]!;
  if (available.length > 1) return available[0]!;

  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, FAL_KEY, or REPLICATE_API_TOKEN.\n" +
