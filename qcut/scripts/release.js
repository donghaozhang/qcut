...
function checkGitStatus() {
  try {
    ...
    if (status.trim()) {
      throw new Error('Working directory is not clean. Please commit your changes first.');
    }
...
function generateChecksums() {
  ...
  try {
    ...
    const checksum = execSync(`powershell "Get-FileHash '${installerPath}' -Algorithm SHA256 | Select-Object -ExpandProperty Hash"`, { encoding: 'utf8' }).trim();
...
`;
    ...
    console.log('⚠️  Could not generate full release notes template, creating basic version');
