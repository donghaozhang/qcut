...
export type PlatformHyperframesVariableValue = string | number | boolean;

export interface PlatformHyperframesRenderProgress {
	renderId: string;
	elementId: string;
	frame: number;
	totalFrames: number;
...
export interface PlatformHyperframesAPI {
	select(): Promise<{
		success: boolean;
		cancelled?: boolean;
		sourcePath?: string;
	...
	registerPreview(options: {
		sourcePath: string;
		variables: Record<string, PlatformHyperframesVariableValue>;
	}): Promise<{
		success: boolean;
		token?: string;
		url?: string;
