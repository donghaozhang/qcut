...
/** POST /api/commands/mergeit — Read mergeit.md and send it to a session */
export async function POST(request: NextRequest) {
	...
	try {
		...
		// Resolve mergeit.md relative to the repo root (dirname of qagent.yaml)
		const repoRoot = dirname(config.configPath);
		const mergeItPath = join(repoRoot, ".claude", "commands", "mergeit.md");
		const instruction = readFileSync(mergeItPath, "utf-8").trim();

		if (!instruction) {
			return NextResponse.json(
				{ error: "mergeit.md is empty" },
				{ status: 422 }
			);
		}
		...
		const msg = status === 404 ? "Resource not found" : "Failed to send mergeit instruction";
		console.error("[mergeit]", raw);
