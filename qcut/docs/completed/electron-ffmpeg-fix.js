...
function demonstrateProblem() {
    ...
    execFile(FFMPEG_PATH, [
        ...
        if (error) {
            ...
            console.log('Unexpected success - you might not be in Electron environment\n');
        }
        demonstrateSolution();
    });
}
    ...
    exec(`cscript //NoLogo "${vbsFile}"`, (error, stdout, stderr) => {
        if (error) {
            ...
            if (fs.existsSync(outputFile)) {
                ...
                console.log(`Output: ${outputFile} (${(stats.size / 1024).toFixed(2)} KB)\n`);
            }
        }
...
function demonstratePowerShell() {
    ...
    exec(`powershell -Command "${psCommand}"`, { maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
        if (error) {
...
`;
    ...
    console.log('3. PowerShell: Good alternative but may have execution policy issues');
