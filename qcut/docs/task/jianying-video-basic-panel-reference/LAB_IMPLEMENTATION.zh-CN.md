...
> 实现日期：2026-08-30；剪映本机缓存 Provider 更新：2026-08-31
>
...
| --- | --- | --- | --- |
| 防闪烁 | 实验室防闪烁 | 是 | 主路径可显式使用剪映 11.3.0 `VideoDeflickerGpuBackend` 本机缓存并生成派生视频；FFmpeg `deflicker` 保留为公开回退 |
...
未点击本机缓存按钮时，防闪烁、运动模糊和本地超分进入同一条 FFmpeg 预览代理、单帧预览和最终导出滤镜链。点击 `使用本机剪映缓存处理` 后，QCut 使用隔离原生宿主处理完整视频、验证并缓存派生 MP4、替换时间线素材，再把 `labDeflicker` 归零，避免导出时二次处理。完整架构和门禁见 [剪映本机缓存防闪烁 Provider](./PRIVATE_RUNTIME_DEFLICKER.zh-CN.md)。
...
- FFmpeg 三项、三种关键帧工具和剪映本机缓存防闪烁都不需要网络；私有宿主由 `sandbox-exec` 显式禁止网络。
...
- 防闪烁已恢复并验证连续帧处理 ABI。ByteNN、UMVFI、VAS、VMB 和剪映眼神模型仍只有较低探针等级证据，没有完整稳定的帧处理 ABI，因此其他 UI 继续使用可验证的 QCut 本地实现，不使用这些私有品牌名。
...
剪映本机缓存防闪烁另有独立可见 UI E2E：真人素材经过按钮处理后 `mediaId` 改变、输出文件超过 100 KB、时间线强度归零、中央预览非空，并保存 UI 截图和状态 JSON。结果：`1 passed (10.7s)`。

## 仍需后续升级

1. 把已经用于私有防闪烁的后台派生媒体缓存模型扩展到运动模糊和超分，长片不应每次拖动都重算。
