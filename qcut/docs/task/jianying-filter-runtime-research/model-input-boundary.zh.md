...
两条宿主路径没有把真实图像的逐值相同张量送入 `tt_skin_seg`。主差异是垂直方向；校正后仍存在较小但真实的预处理差异。纯色平场已经排除颜色路径，坐标坡度又排除了宏观 resize 坐标体系；剩余差异必须依赖空间变化，候选范围收窄到亚像素精度、插值 kernel、边界处理和输出取整。

这次结果把差异定位到模型之前，不能再把全部误差归因于 segmentation 状态、mask 后处理或 LUT 混合。下一轮若继续，应一次只验证插值/取整中的一个变量；本记录没有继续展开 kernel 实验。
