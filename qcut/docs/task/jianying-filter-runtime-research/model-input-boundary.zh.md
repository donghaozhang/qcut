...
下一轮只验证输出取整。保持水平 half-pixel 坐标、854x480 输入和 224x128 模型张量不变，生成五组不同斜率与相位的三角坡度。对每个输出位置根据相邻两个 8-bit 源像素精确计算双线性结果，再分别套用向下取整、四舍五入和向上取整，与模型入口张量比较。

前三组低频坡度的结果最能隔离取整；高频坡度会放大亚像素坐标或 kernel 的微小差异：

| 输入 | Low-level 最佳规则（MAE / 完全命中） | Swing 最佳规则（MAE / 完全命中） |
| --- | --- | --- |
| slope 1, phase 0 | 四舍五入：`0.210 / 79.02%` | 向下取整：`0.263 / 73.66%` |
| slope 2, phase 17 | 四舍五入：`0.161 / 85.27%` | 向下取整：`0.272 / 72.77%` |
| slope 3, phase 43 | 四舍五入：`0.263 / 76.79%` | 向下取整：`0.241 / 76.79%` |

合并这三组的 672 个水平位置后，Low-level 与 Swing 有 423 个相同、215 个为 `Low-level = Swing + 1`、30 个为 `Low-level = Swing - 1`，另有 4 个较大正差。也就是说，`94.94%` 的位置符合“四舍五入相对向下取整只能产生 0 或 +1”的必要形态；在 249 个非零差异中，`86.35%` 正好是 `+1`。

这确认了取整规则是低频输入差异的主要来源，但不是唯一来源。纯取整模型仍解释不了 `5.06%` 的位置；斜率 5 和 7 时，最佳 MAE 分别上升到 Low-level `0.429 / 0.616`、Swing `0.339 / 0.451`，两路差值相对取整模型的最大残差达到 6 和 8。该趋势说明亚像素采样精度、插值 kernel 或纹理边界行为仍在高频区域产生可测差异。

本实验只解释 Low-level 与 Swing 的预处理差异，不确定剪映 UI 使用哪一种取整。当前 Low-level 最终输出对 UI 的 `37.331 dB` 仍高于 Swing 的 `32.669 dB`，因此不能为了张量接近 Swing 而改变 Low-level；任何实现调整都必须以 UI 最终帧是否超过 `37.331 dB` 为准。

## 结论

两条宿主路径没有把真实图像的逐值相同张量送入 `tt_skin_seg`。主差异是垂直方向；校正后仍存在较小但真实的预处理差异。纯色平场已经排除颜色路径，坐标坡度又排除了宏观 resize 坐标体系；多斜率 A/B 确认输出取整是低频差异的主要来源，但高频残差证明亚像素精度、插值 kernel 或边界处理仍未对齐。

这次结果把差异定位到模型之前，不能再把全部误差归因于 segmentation 状态、mask 后处理或 LUT 混合。下一轮若继续，应只验证插值 kernel，不再重复颜色、坐标体系或取整枚举；本记录没有继续展开 kernel 实验。
