...
轨道能力不是永远固定的：应用资源明确说明“自由层级”开启并保存后不可关闭；它是否会取消轨道锁定/隐藏限制仍未验证。“素材混排”允许不同类型素材进入同一轨道，但要求先开启自由层级，而且开启后不可关闭。因此 QCut 的导入器至少要保存以下项目级 profile：
...
- 明确指定的目标轨道被锁定时，命令必须原子失败；仅因派生波纹或广域操作波及的锁定轨道则跳过。两种情况都不能提交半套状态。
...
要求生成、写后重读、真实打开、保存、重开和导出；视觉层级、音频混合、转场接缝和 source sampling 都要有 oracle。不得在真实用户账号中运行写操作；应使用隔离测试账号或 VM。
