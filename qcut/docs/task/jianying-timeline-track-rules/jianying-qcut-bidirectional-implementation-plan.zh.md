...
| JYI-009 Import bundle | ✅ 已完成 | 2026-08-04 | `editor-core/draft-interop/import-bundle.ts`（共享 schema，纯数据无 node 依赖）+ `jianying-draft-import/qcut-import-bundle-builder.ts`。单一共享 schema：`QCutImportBundleV1` 打包 document + timelinePlan + resourceStaging + `internalIdBySemanticId`，live bridge 与 offline inbox 都必须过同一个 `parseQCutImportBundleV1`（fail-closed 结构校验 + 交叉引用校验：staging 只许引用 document 声明的资源、plan 节点必须有确定性内部 ID、plan 元素必须引用已 staging 资源、stagingKey 不得形似路径/含穿越/NUL）。Digest：editor-core 定义规范序列化（递归键排序 + digest 字段占位归零），runtime 用 node:crypto 计算/验证——结构合法但内容被篡改的 bundle 能过 parse 但 digest 必然失配（有测试）。确定性内部 ID：`deriveImportInternalId` 以 requestFingerprint 为种子（纯 TS，复用 `createDeterministicJianyingId`），同源重 plan 不同 token 下 ID 完全一致。Conflict policy：`projectName: rename|fail`（默认 rename），未知策略拒绝。builder 自检：产出前跑与 renderer 相同的 parser。测试 [`import-bundle.test.ts`](../../../packages/jianying-draft-import/src/__tests__/import-bundle.test.ts)（10 用例，fixture 走 builder→normalize→plan→artifact→bundle 全链路） |
...
| JYI-010 Renderer storage transaction | ✅ 已完成 | 2026-08-04 | `apps/web/src/lib/storage/{import-journal,import-staging-adapter,import-recovery}.ts` + `lib/jianying-draft/qcut-import-transaction.ts`。Journal：任何项目数据写入前先落 intent 记录（Electron→IndexedDB→localStorage 探测链，可注入 adapter 测试），phase staging→published，publish 验证后才删除。Staging session：媒体字节与 timeline 写在"无 project 记录指向"的新 id 下，`publishProject` 是唯一可见化步骤（单次、id 匹配校验、写后 readback）；rollback 只删 journal 归属本次导入的数据（media+双 timeline+project）。Transaction：renderer 重跑同一个 `parseQCutImportBundleV1` + WebCrypto 字节级 digest 验证（无 WebCrypto 即 fail-closed）→ 配额检查 → 项目身份（以源文件哈希为种子确定性派生，非 bundleDigest——policy 变化不换身份；冲突按 rename/fail 策略）→ journal begin → 逐资源 stage（缺 resolved payload 直接拒，写前零副作用）→ build tracks（最小 MediaElement，playbackRate 承接 speed）→ 双写 timeline → `verifyStaged` 重读校验 → publish 最后写 → readback → journal 清除；staging/publish 中途失败全量回滚。Recovery：启动时扫 journal——staging 记录回滚清理、published+项目可读补完 bookkeeping、published+项目丢失清残留、无法恢复的保留到下次启动。bundle 的 internalIdBySemanticId 扩展为覆盖 staged 资源（builder+parser 同步收紧）。测试 [`import-transaction.test.ts`](../../../apps/web/src/lib/storage/__tests__/import-transaction.test.ts)（12 用例:全链路发布次序/确定性项目 ID/篡改 digest 零写入/缺 payload 零写入/staging 失败回滚/publish 失败回滚/rename+fail 冲突策略/单次 publish/id 失配拒绝/恢复三态） |
...
| JYI-015 同版本 envelope/写回 | 🟨 部分完成 | 2026-08-04 | 导入项目现已持久化版本化来源绑定与可选 `ForeignDraftEnvelope` 元数据。Live commit 仅封装有证据的 allowlist 字节；renderer 校验与源文件绑定的 metadata，在 project 发布前加密，仅保存不透明 payload 引用，并在失败时回滚 project/media/timeline/envelope。Offline inbox 会剥离 capture 字节。真实 CapCut 8.1.1 保存草稿（`new_version` 从 canonical `159.0.0` 迁移到实测 `179.0.0`）已精确识别，并从 25 个发现文件中只捕获 `draft_info.json`：源 14,273B、6 个 bindings、payload 19,115B、零 warning。证据：[`capcut-8.1.1-envelope-capture-2026-08-04.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-envelope-capture-2026-08-04.json)。写回基础层已新增共享 payload V1 校验（严格路径顺序、base64、长度和 SHA-256）、renderer key-version/解密后完整性门禁、带旧值前置条件和 unknown ownership 冲突检查的标量 JSON Pointer patcher，以及 CapCut 8.1 核心 video/audio 时间域 planner；新增/删除/跨轨移动、换素材和变速均 fail-closed，共 26 个聚焦测试。真正文件系统保存事务、真实 unknown sentinel 的 8.1 open/save/reopen 以及 profile 可写开关仍受 JYR-001/JYR-005~007 门禁。 |
| JYI-016 ~ JYI-018 | ⬜ 未开始 | | 特性 mapper、语义/逐帧/音频往返 E2E 与大工程/恢复仍未完成。 |
...
| JYR-002 ~ JYR-008 研究门禁 | 🟨 部分完成 | 2026-08-04 | JYR-003 已记录 CapCut 8.1.1 精确保存迁移 `new_version: 159.0.0 → 179.0.0`；JYR-006 目前仅凭真实 App 文件访问证据准入根 `draft_info.json`。writer 所有权、伴随引用闭包、完整 sidecar 需求、许可与保存事务顺序仍未知，继续阻止写回。 |
