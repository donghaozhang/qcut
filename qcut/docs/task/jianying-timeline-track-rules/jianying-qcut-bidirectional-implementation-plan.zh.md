...
| JYI-003 Profile registry/detection | ✅ 已完成 | 2026-08-13 | `jianying-draft/profiles/` + `import/profile-detection.ts`。Profile 契约按操作使用 none/fixture/candidate/stable 分级，`isDraftProfileWritable` 只检查 `sameProfileWriteback: stable`，不会因 inspect/import stable 而误开写回。注册表包含 synthetic 5.9、CapCut 8.1 和 JianYing 11.3 beta2/3/4；beta2/3 仍是非生产 candidate，beta4 仅将真实 E2E 覆盖的 inspect/import 升为 stable/production，所有 JianYing 11.3 写能力仍为 none。Detection 使用 app metadata、schema/new_version、top-level key 包含和文件布局四路独立信号；缺精确 app metadata 时 ambiguous 且禁止写，加密 content 为终态，纯文件名不作判据。测试 [`profile-detection.test.ts`](../../../packages/editor-core/src/__tests__/profile-detection.test.ts) 覆盖注册状态、精确 beta2/3/4 检测、未知版本、歧义、unsupported、encrypted 与写回门禁。 |
...
| JYI-014J JianYing 11.3 beta4 最小生产导入 | ✅ 已完成 | 2026-08-13 | 对真实 beta4 牺牲工程的唯一明文复合 subdraft 执行 live CLI 导入，显式接受 2 个 opaque warning；全新隔离 QCut 用户目录成功持久化 1920×1080/30fps 项目、1 个视频资源和 1 条 media track，元素保持 `startTime=0.4s`、`duration=8s`。随后 `qcut editor interop import-snapshot` 对 renderer 存储连续双读，复验 4,511,472B 媒体长度与 SHA-256，输出零绝对路径；导入前后活动 subdraft 与 opaque parent hash 均未变化。beta4 的 `inspect/import` 因而升为 stable/production；这只覆盖核心单视频 compound 子集。JYR-007 同时证明写回失败，所以 `sameProfileWriteback`、`crossProfileExport` 仍为 `none`，`realAppVerified` 仍为 false，`qcut draft export` 实测以 `profile-not-writable` 阻断。私有原始草稿与收据只保存在本机 evidence 目录，不进仓库。 |
| JYI-014K JianYing 11.3 beta4 静态文字导入 | ✅ 最小节点完成 | 2026-08-13 | 真实 App 创建的单普通文字 compound subdraft 已接入同一生产导入链：`mixed` 纯文字轨被识别为 text；单 UTF-16 style、单位变换、透明背景、缺省描边/阴影及仅含空 `material_animations` 的伴随引用可映射为 QCut TextElement；有效动画、花字/text effect、关键帧和多样式仍保持 opaque/blocked。真实 CLI `plan` 为 exact、`canCommit=true`、零 blocker 和 1 个明确 downgrade；不接受 warning 时 offline commit 与 live import 都拒绝，精确接受后既可原子排入隔离 inbox，也可由匹配版本的运行中 QCut 直接持久化。可信 renderer 双读确认 1920×1080/30fps、1 条 text track、1 个 3 秒透明背景文字元素、零媒体和零 skipped；`qcut draft verify-roundtrip` 同时证明活动创建时 subdraft 的 no-op 字节不变，并明确 `writebackPerformed=false`、`targetAppPersistenceVerified=false`。真实 App 还证明 compound subdraft 是创建时快照：后续嵌套编辑不会更新旧快照，旧、新 snapshot 可并存，因此从工程根发现多个候选时必须显式选择，不能猜 active owner。该能力只宣称有警告的静态文字导入，不宣称字体逐帧一致、文字写回或 opaque parent 所有权；私有草稿、截图和路径化证据仅留本机、不进仓库。 |
...
| JYI-016 特性 mapper | 🟨 部分完成 | 2026-08-13 | 已完成四条 fail-closed 导入子链。其一是证据门禁下的 CapCut 8.1 原生 `Dissolve`：仅在精确身份、相邻 seam、正整数时长及两侧片段承载能力均成立时映射为 QCut `dissolve/easeInOut`。其二是共享静态单样式普通文字 mapper：仅接受完整 UTF-16 单 style range、有限画布/变换、单位缩放、无翻转、无 keyframe/有效 animation/装饰效果；JianYing beta4 只额外允许真实样本证明为空的 `material_animations` 伴随引用和缺省描边/阴影。它映射文字内容、字体、字号、填充、对齐、粗斜体/下划线、字距/宽度、位置/旋转/透明度、单描边、单阴影和背景；多样式、字幕语义、动态/花字及畸形值一律 blocked/opaque，不静默压平。其三是仅用于 JianYing beta4 的静态本地音频 mapper：只有全长 `extract_music`、默认播放/音量状态和五类实测默认伴随对象可 exact；trim、变速、音量、循环、反转、音调、人声/声道处理及关键帧均 fail closed 为 opaque。其四是仅用于 JianYing beta4 的静态本地视频 mapper：只有全长片段，track/render 索引与来源轨道顺序一致，且材料、片段和六类伴随对象都符合真实 App 默认值时才标 exact；trim、裁剪、变换、播放、可见性、调色、关键帧、算法、蒙版和伴随处理均 fail closed 为 opaque。CapCut 8.1 文字因缺真实 render receipt 保持 downgrade；JianYing beta4 文字因系统字体替换和创建时 subdraft 可能过期而保持 downgrade，均不宣称逐帧 exact；JianYing beta4 音频/视频 exact 也只表示结构与持久化映射精确，不表示剪映导出已完成频谱或逐帧一致性验证。bundle parser 强制轨道/元素类型一致并校验 timelinePlan 是 document 的确定性投影，renderer 可持久化零媒体文字、音频 payload 和有序多视频时间线；定向 mapper/session 回归、全仓 `check-types` 与 Electron/import-runtime build 均需保持通过。调色、蒙版、关键帧、富文本/动态文字、音频处理、变换/裁剪视频和专有转场 mapper 仍未完成。 |
...
当前 `verify-roundtrip` 的成功范围仅是 `active-subdraft-noop`：证明导入投影在不修改 QCut 时间线时可重新生成 byte-identical 的活动明文 subdraft；返回值必须同时显示 `writebackPerformed: false` 与 `targetAppPersistenceVerified: false`。它不等价于剪映真实保存/重开，也不能解除 JYR-007。`export` 对未达到 stable writeback 的 profile 必须以 `profile-not-writable` 失败，不能选择“最接近版本”或绕过门禁。
...
| JianYing 11.3.0-beta4 plaintext compound subdraft | stable inspect | 核心单视频子集 stable | blocked | blocked | import 已真实验证；opaque parent ownership 未解 |
| 其他 JianYing 11.x 新格式 | read-only 起步 | 无证据时 blocked | blocked | blocked | unresolved/encrypted |
...
| JYR-007 | parent draft、subdraft、compound timeline 的 ID binding、版本和保存职责 | beta4 已证明只修改活动明文 subdraft 会在保存时被 opaque parent 回滚；最终 owner 和合法写入接口仍未知 | 在新 disposable case 上追踪一次剪映原生复合片段 timing 修改的文件访问顺序；只记录路径类别与操作，不读取 key store 或调用私有 crypto ABI | JianYing 11.3 compound 只允许导入与 opaque 保留；same-profile writeback 和 cross-profile export 均 blocked |
