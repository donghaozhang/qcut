...
| JYI-016 特性 mapper | 🟨 部分完成 | 2026-08-04 | 已完成证据门禁下的 CapCut 8.1 原生 `Dissolve` 导入：转场成为独立轨道语义节点，精确身份、相邻 seam、正整数时长及两侧片段承载能力均通过后，映射为 QCut `dissolve/easeInOut`；未知、重复或不连续转场明确 downgrade/blocked，且不再拖累两侧媒体片段。bundle parser 校验同轨端点和确定性 ID，renderer 事务持久化重映射后的转场。相关 5 个测试文件 56 个聚焦用例通过。文字、调色、蒙版、关键帧和专有转场 mapper 仍未完成。 |
| JYI-017 ~ JYI-018 | ⬜ 未开始 | | 语义/逐帧/音频往返 E2E 与大工程/恢复仍未完成。 |
