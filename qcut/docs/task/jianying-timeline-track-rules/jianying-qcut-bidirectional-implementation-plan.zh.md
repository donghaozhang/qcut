...
| JYI-016 特性 mapper | 🟨 部分完成 | 2026-08-13 | 已完成两条 fail-closed 导入子链。其一是证据门禁下的 CapCut 8.1 原生 `Dissolve`：仅在精确身份、相邻 seam、正整数时长及两侧片段承载能力均成立时映射为 QCut `dissolve/easeInOut`。其二是共享静态单样式普通文字 mapper：仅接受完整 UTF-16 单 style range、有限画布/变换、单位缩放、无翻转、无 keyframe/有效 animation/装饰效果；JianYing beta4 只额外允许真实样本证明为空的 `material_animations` 伴随引用和缺省描边/阴影。它映射文字内容、字体、字号、填充、对齐、粗斜体/下划线、字距/宽度、位置/旋转/透明度、单描边、单阴影和背景；多样式、字幕语义、动态/花字及畸形值一律 blocked/opaque，不静默压平。CapCut 8.1 因缺真实文字 render receipt 保持 downgrade；JianYing beta4 虽有真实创建与 live QCut 持久化证据，仍因系统字体替换和创建时 subdraft 可能过期而保持 downgrade，均不宣称逐帧 exact。bundle parser 强制轨道/元素类型一致并校验 timelinePlan 是 document 的确定性投影，renderer 可在零媒体 payload 时持久化 QCut TextElement；定向 mapper/session 回归、全仓 `check-types` 与 Electron/import-runtime build 均需保持通过。调色、蒙版、关键帧、富文本/动态文字和专有转场 mapper 仍未完成。 |
