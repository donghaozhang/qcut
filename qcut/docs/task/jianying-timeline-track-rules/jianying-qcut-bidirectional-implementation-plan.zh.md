...
| JYI-017 语义/逐帧/音频 E2E | 🟨 部分完成 | 2026-08-05 | 语义、采样帧、native/preview、音频链保持不变。新增 [`qcut-import-verification.ts`](../../../scripts/capcut-e2e/qcut-import-verification.ts) 及拆分的 contract/snapshot 模块：共享 parser 先验证 bundle 结构和 `document→timelinePlan` 确定性投影，再验证 bundle digest；QCut 快照媒体以 O_NOFOLLOW + fstat/lstat 身份门禁流式计算 byteLength/SHA-256，完整 tracks 复用 editor-core verifier 比较，project name/FPS/geometry 单独门控；所有 pass/fail/not-comparable 分支均可写出仅含 hash/count/JSON-pointer issue 的零绝对路径 manifest。真实临时媒体文件测试覆盖 exact pass、timeline/media 篡改 fail、digest/快照无效 not-comparable、CLI 和路径脱敏。总编排 [`roundtrip-case.ts`](../../../scripts/capcut-e2e/roundtrip-case.ts) 现要求 `--qcut-import-bundle` 与 `--qcut-import-snapshot`，新增必经 `qcut-import` gate，再合并 source→roundtrip 语义、reference→QCut 两份 native export、两套 preview PNG 与音频证据；任一导入/比较 fail 优先，其次 not-comparable，再是 unverified，只有全部 gate verified 才 pass。相关 3 个脚本测试文件 15 用例通过。**仍缺**：从真实 renderer 自动导出该原始落盘快照的可信 capture/receipt、keyframe 三点采样、Alpha/Geometry/Temporal 与测试 tone 频谱、可信 App/资源/导出设置 provenance，以及真实 CapCut 8.1 四路输出和逐 feature/profile 阈值校准（本机为 9.1，不能替代 8.1 证据）。 |
