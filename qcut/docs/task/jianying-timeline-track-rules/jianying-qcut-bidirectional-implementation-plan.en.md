...
| JYI-015 Same-profile envelope/writeback | 🟨 Partial | 2026-08-04 | Imported projects now persist a versioned source binding and optional `ForeignDraftEnvelope` metadata. Live commits package only evidence-allowlisted bytes; the renderer validates source-bound metadata, encrypts before project publication, stores only the opaque payload reference, and rolls back project/media/timeline/envelope state on failure. The offline inbox strips capture bytes. A real CapCut 8.1.1 saved draft (`new_version` migrated from canonical `159.0.0` to observed `179.0.0`) was detected exactly and captured only `draft_info.json` from 25 discovered files: 14,273 source bytes, 6 bindings, 19,115-byte payload, zero warnings. Evidence: [`capcut-8.1.1-envelope-capture-2026-08-04.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-envelope-capture-2026-08-04.json). The writeback foundation now has one shared payload-V1 verifier (strict entry order, base64, byte length, and SHA-256), renderer key-version and post-decryption integrity gates, a scalar JSON Pointer patcher with old-value preconditions and unknown-ownership conflict checks, and a CapCut 8.1 core video/audio timing planner. Add/delete/cross-track moves, resource replacement, and speed changes fail closed; 26 focused tests cover these boundaries. The filesystem save transaction, real 8.1 unknown-sentinel open/save/reopen, and writable profile switch remain gated on JYR-001/JYR-005–007. |
...
| JYR-002 – JYR-008 research gates | 🟨 Partial | 2026-08-04 | JYR-003 records the exact CapCut 8.1.1 save migration `new_version: 159.0.0 → 179.0.0`; JYR-006 currently admits only root `draft_info.json` from real-app access evidence. Writer ownership, companion-reference closure, complete sidecar needs, licensing, and transaction order remain unresolved and continue to gate writeback. |
