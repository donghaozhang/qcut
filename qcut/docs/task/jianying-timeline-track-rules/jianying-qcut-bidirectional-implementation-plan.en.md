...
| JYI-015 Same-profile envelope/writeback | 🟨 Partial | 2026-08-05 | Imported projects persist a versioned source binding, normalized `DraftInteropDocumentV1` baseline, and optional `ForeignDraftEnvelope` metadata; raw source bytes remain only in the OS-keychain-wrapped encrypted payload. A receipt-bound real CapCut 8.1.1 saved draft (`new_version` observed at `179.0.0`) captures only `draft_info.json` from 25 discovered files: 14,273 source bytes, 6 bindings, a 19,115-byte payload, and zero warnings. Evidence: [`capcut-8.1.1-envelope-capture-2026-08-04.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-envelope-capture-2026-08-04.json). The writeback chain includes strict payload/key-version/SHA-256 verification, scalar JSON Pointer patches with old-value and unknown-ownership conflict guards, the core video/audio timing planner, unknown-preserving preparation, renderer orchestration, and trusted Electron IPC bound to a short-lived directory-selection token. The planner now consumes a dedicated pathless timing snapshot (tracks plus playback-aware durations), so imported IndexedDB media no longer needs an export-only filesystem path before writeback can be prepared. The main-process writer transactionally replaces exactly four active mirrors, never touches `.bak`, and provides fsync, independent rollback copies, journal recovery, `.locked`, symlink, TOCTOU, and source-hash gates; add/delete/cross-track moves, resource replacement, and speed changes fail closed. The new receipt-bound isolated-copy verifier injected a controlled unknown segment sentinel, ran production `inspect → plan → commit`, applied four timing leaves, and proved all four active mirrors matched, both `.bak` mirrors and the original source stayed unchanged, only the planned JSON pointers changed, the sentinel survived, and recovery returned `none`. Path-free evidence: [`capcut-8.1.1-same-profile-writeback-2026-08-05.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-same-profile-writeback-2026-08-05.json). Its verdict intentionally remains `unverified`: the exact CapCut 8.1 application still must open, save, and reopen the written copy. A user-facing QCut UI/CLI command and the writable profile switch also remain missing; CapCut 9.1 cannot substitute for the 8.1 gate. |
