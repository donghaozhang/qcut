...
**Date:** 2026-08-04  
**Last verified revision:** `a5b6acc49`
...
| JYI-017 Semantic/frame/audio E2E | 🟨 Partial | 2026-08-05 | The semantic core and runner remain intact: `diffDraftInteropDocuments` classifies user-visible source/roundtrip differences as breaking/tolerable/info, while the script reuses the production discover→snapshot→detect→normalize path and emits a hash-bound manifest with no absolute paths. New deterministic [`frame-sample-plan.ts`](../../../scripts/capcut-e2e/frame-sample-plan.ts) covers project first/last, clip and static-text boundaries, transition seam before/middle/after, the longest stable interval, and fixed-seed random frames, deduplicated under a 256-sample cap; keyframes remain explicitly unsupported and transition windows remain semantic-seam candidates. [`frame-comparison.ts`](../../../scripts/capcut-e2e/frame-comparison.ts) uses bundled FFmpeg/FFprobe 8.1.2 to gate FPS, decoded frame count, geometry, and plan coverage before per-sample RGB RMSE/MAE/p95/max plus bilateral SHA-256. [`preview-frame-comparison.ts`](../../../scripts/capcut-e2e/preview-frame-comparison.ts) requires fixed `frame-XXXXXXXX.png` names, compares immutable byte snapshots, and makes missing frames not-comparable. Audio comparison covers single-stream layout, sample rate, duration, EBU R128, silence boundaries, and per-channel difference peak/RMS. [`roundtrip-case.ts`](../../../scripts/capcut-e2e/roundtrip-case.ts) fixes source→roundtrip and reference→QCut roles and combines semantic evidence, both native exports, both preview PNG sets, and audio. Failure outranks not-comparable, which outranks unverified; pass requires every gate to be verified. A four-second synthetic smoke through the real code path produced semantic identical, RMSE 0 at all 11 planned native/preview frames, and exact audio, but correctly returned unverified/exit 2 because thresholds, keyframe/transition coverage, and app/resource/export-setting provenance remain candidates. **Missing**: `qcut-import-verification`, keyframe triplet sampling, Alpha/Geometry/Temporal and test-tone spectrum metrics, trusted provenance receipts, and real CapCut 8.1 four-way outputs with per-feature/profile threshold calibration (this machine runs 9.1 and cannot substitute for 8.1 evidence). |
...
Current four-way orchestration command:
