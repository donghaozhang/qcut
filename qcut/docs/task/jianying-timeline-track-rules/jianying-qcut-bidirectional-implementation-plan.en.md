...
| JYI-016 feature mappers | 🟨 Partial | 2026-08-04 | Added evidence-gated CapCut 8.1 native `Dissolve` import. Transitions are independent track-level semantic nodes and map to QCut `dissolve/easeInOut` only after exact identity, adjacent seam, positive integer duration, and adjacent-segment capacity checks. Unknown, duplicate, or discontinuous transitions are explicitly downgrade/blocked without degrading their media segments. The bundle parser validates same-track endpoints and deterministic IDs, and the renderer transaction persists remapped transitions. The focused suite passes 56 cases across 5 files. Text, color, mask, keyframe, and proprietary-transition mappers remain. |
