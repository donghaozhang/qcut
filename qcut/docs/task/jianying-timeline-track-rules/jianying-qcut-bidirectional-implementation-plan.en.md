...
| JYI-017 Semantic/frame/audio E2E | 🟨 Partial | 2026-08-05 | The semantic, sample-frame, native/preview, and audio paths remain intact. New [`qcut-import-verification.ts`](../../../scripts/capcut-e2e/qcut-import-verification.ts), split contract, and snapshot modules first run the shared bundle parser and deterministic `document→timelinePlan` check, then verify the bundle digest. QCut media is streamed through O_NOFOLLOW plus fstat/lstat identity gates for byteLength/SHA-256; complete tracks reuse the editor-core materialization verifier, while project name/FPS/geometry have explicit gates. Every pass/fail/not-comparable path can emit a manifest containing only hashes, counts, and JSON-pointer issues with no absolute paths. Real temporary-file tests cover exact pass, timeline/media tamper failure, digest/invalid-snapshot not-comparable, CLI parsing, and path redaction. [`roundtrip-case.ts`](../../../scripts/capcut-e2e/roundtrip-case.ts) now requires `--qcut-import-bundle` and `--qcut-import-snapshot`, adds a mandatory `qcut-import` gate, then combines source→roundtrip semantics, both reference→QCut native exports, both preview sets, and audio. Failure outranks not-comparable, which outranks unverified; pass requires every gate to be verified. Fifteen cases across the three related script test files pass. **Remaining**: trusted automatic capture/receipt of this raw persisted snapshot from the real renderer, keyframe triplet sampling, Alpha/Geometry/Temporal and test-tone spectrum metrics, trusted app/resource/export-setting provenance, and real CapCut 8.1 four-way outputs with per-feature/profile threshold calibration (this machine runs 9.1 and cannot substitute for 8.1 evidence). |
