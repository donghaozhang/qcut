...
**Date:** 2026-08-04  
**Last verified revision:** `d04b0cce7`
...
| JYI-017 Semantic/frame/audio E2E | 🟨 Partial | 2026-08-05 | The semantic, sample-frame, native/preview, and audio paths remain intact. The trusted import-evidence transport is now wired end to end: the renderer double-reads persisted IndexedDB project/timeline/media state, rejects mutation between reads, and streams media SHA-256; Electron accepts only the active window's main-frame response, while HTTP/preload/utility bridges share strict validation. `qcut editor interop import-snapshot` captures a path-free, no-overwrite, mode-0600 snapshot bound to project ID and bundle digest. Import manifest schema 2 now gates complete media/tracks/project state plus bundle digest, import ID, profile ID, and trusted capture source. A legacy schema-1 absolute-`sourcePath` snapshot remains diagnostic-only and is not-comparable even when exact. [`roundtrip-case.ts`](../../../scripts/capcut-e2e/roundtrip-case.ts) keeps the mandatory `qcut-import` gate before semantic, two native-export, two preview, and audio evidence. Nineteen verifier/roundtrip tests pass; renderer, IPC, HTTP, and CLI have separate trust-boundary coverage; repository-wide `check-types` passes. **Remaining**: a trusted capture receipt from a real running QCut project, keyframe triplet sampling, Alpha/Geometry/Temporal and test-tone spectrum metrics, trusted app/resource/export-setting provenance, and real CapCut 8.1 four-way outputs with per-feature/profile threshold calibration (this machine runs 9.1 and cannot substitute for 8.1 evidence). |
