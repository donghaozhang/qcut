...
| JYI-016 feature mappers | 🟨 Partial | 2026-08-13 | Three fail-closed import slices are implemented. First, evidence-gated CapCut 8.1 native `Dissolve` maps to QCut `dissolve/easeInOut` only after exact identity, adjacent seam, positive integer duration, and adjacent-segment capacity checks. Second, a shared static single-style plain-text mapper accepts only one style covering the full UTF-16 range, finite canvas/transform values, unit scale, no flip, and no keyframe, effective animation, or decorative effect. JianYing beta4 additionally permits only its real-fixture-proven empty `material_animations` companion and omitted stroke/shadow arrays. It maps content, family, size, fill, alignment, weight/style/underline, spacing/width, position/rotation/opacity, one stroke, one shadow, and background. Multi-style, subtitle semantics, dynamic/decorative text, and malformed values are blocked/opaque rather than silently flattened. Third, a JianYing-beta4-only static local-audio mapper marks exact only a full-length `extract_music` clip with default playback/volume state and all five real-fixture default companions; trim, speed, volume, loop, reverse, tone, vocal/channel processing, and keyframes fail closed to opaque. CapCut 8.1 text remains downgrade because no real render receipt exists. JianYing beta4 text remains downgrade because system-font substitution and stale creation-time subdraft ownership can differ from the live project; neither text profile is claimed frame-exact. JianYing beta4 audio exactness likewise means exact structural and persisted-state mapping, not yet spectrum-verified JianYing audio export. The bundle parser enforces track/element agreement and deterministic projection, while the renderer persists either zero-media text or a two-track project with an audio payload. Focused mapper/session regression, repository-wide `check-types`, and the Electron/import-runtime build must remain green. Color, mask, keyframe, rich/dynamic text, audio-processing, and proprietary-transition mappers remain. |
