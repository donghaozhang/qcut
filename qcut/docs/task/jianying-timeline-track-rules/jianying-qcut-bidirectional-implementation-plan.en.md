...
**Date:** 2026-08-04  
**Last verified revision:** `effe707d9`
...
| JYI-014 CapCut 8.1 production import | ✅ Done | 2026-08-04 | The core video/audio subset completed a real chain: CapCut 8.1.1 open, autosave, exit, and reopen; CLI exact detection of the saved 23-file snapshot with 2 tracks / 2 segments / 2 resources all exact and resolved, zero warnings or blockers; offline commit consumed by QCut desktop, project count 87→88, and the opened project still held the 8-second Video/Audio timeline and two clips after reload. Per-file source verification found zero import-side mutations. Redacted evidence: [`capcut-8.1.1-core-media-import-2026-08-04.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-core-media-import-2026-08-04.json). Only profile `import` advances to stable/production; native export, frame/audio comparison, and same-profile writeback remain unverified, so `realAppVerified` stays false. |
| JYI-015 Same-profile envelope/writeback | 🟨 Partial | 2026-08-04 | Imported projects now persist a versioned source binding, the normalized `DraftInteropDocumentV1` baseline, and optional `ForeignDraftEnvelope` metadata; raw source bytes remain only in the OS-keychain-wrapped encrypted payload. A real CapCut 8.1.1 saved draft (`new_version` migrated from canonical `159.0.0` to observed `179.0.0`) was detected exactly and captured only `draft_info.json` from 25 discovered files: 14,273 source bytes, 6 bindings, 19,115-byte payload, zero warnings. Evidence: [`capcut-8.1.1-envelope-capture-2026-08-04.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-envelope-capture-2026-08-04.json). The writeback chain now includes strict payload/key-version/SHA-256 verification, scalar JSON Pointer patches with old-value preconditions and unknown-ownership conflicts, the core video/audio timing planner, an unknown-preserving preparation coordinator, renderer orchestration, and trusted Electron IPC bound to a short-lived user directory-selection token. The main-process writer transactionally replaces exactly four active content mirrors, never touches `.bak`, and provides fsync, independent rollback copies, journal recovery, CapCut process/`.locked`, symlink, TOCTOU, and source-hash gates; add/delete/cross-track moves, resource replacement, and speed changes fail closed. Eighty-two focused tests across 11 files and the complete repository `check-types` pass. A real CapCut 8.1 unknown-sentinel open/write/reopen/save receipt, user-facing UI/CLI commands, and the writable profile switch remain missing. This machine currently has CapCut 9.1, which cannot substitute for 8.1 evidence, so the status remains partial. |
...
| JYI-017 Semantic/frame/audio E2E | 🟨 Partial | 2026-08-05 | Pure core landed: `editor-core/draft-interop/semantic-diff.ts` — `diffDraftInteropDocuments` compares two `DraftInteropDocumentV1`s (source vs re-import, pre- vs post-writeback) and classifies every difference three ways: breaking (lost/changed user-visible meaning: missing/extra nodes, geometry, link sets, both-sides-known sha256 mismatch), tolerable (timing/speed drift inside caller-declared thresholds; `halfFrameToleranceUs` provides the natural half-frame threshold), info (environment-dependent differences like relink renames). Semantic coverage now includes every visible static-text field plus stroke/background/shadow, and transition presence, type, endpoints, and duration. Nodes are compared by semantic ID so array reordering creates no false difference. Assessment-side fields (capability/issues/resource status/foreignRef) remain excluded by design; missing text/nested style/transition nodes fail closed as breaking without exposing bindings. Pure, deterministic, JSON-serializable. Tests: [`semantic-diff.test.ts`](../../../packages/editor-core/src/__tests__/semantic-diff.test.ts) (14 cases). [`scripts/capcut-e2e/semantic-diff.ts`](../../../scripts/capcut-e2e/semantic-diff.ts) runs the exact import pipeline (discover→snapshot→detect→normalize) over two local draft directories, diffs at the half-frame threshold, and emits a hash-bound, zero-absolute-path evidence manifest (identical/tolerable/breaking/not-comparable verdicts + exit codes). [`scripts/capcut-e2e/audio-comparison.ts`](../../../scripts/capcut-e2e/audio-comparison.ts) now uses the pinned bundled FFmpeg/FFprobe 8.1.2 toolchain to compare one audio stream's layout, sample rate, duration, EBU R128 integrated loudness/LRA/true peak, silence boundaries, and per-channel difference peak/RMS. Its manifest retains only input byte counts/SHA-256 and path-free tool identity; pass/fail/not-comparable return 0/1/2. Thresholds remain explicitly `candidate-unverified`: the real six-second 48 kHz fixture passes exactly, a half-volume candidate fails with roughly -24/-27 dBFS difference, and video-only inputs are not comparable; six new unit tests cover probes, metric parsers, gates, and CLI parsing. **Missing**: `roundtrip-case`/`qcut-import-verification` orchestration, a general frame oracle, test-tone spectrum in the comparison manifest, and real CapCut 8.1 four-way outputs plus threshold calibration (this machine runs 9.1, so evidence remains gated). |
| JYI-018 Scale/recovery hardening | 🟨 Partial | 2026-08-05 | Scale: [`draft-interop-scale.test.ts`](../../../packages/editor-core/src/__tests__/draft-interop-scale.test.ts) runs 10k segments (10 tracks × 1000 + 20k materials) through the full pure pipeline read→validate→normalize→map→diff→bundle-parse with zero issues and zero RESTRICTED path leakage; per-stage count assertions plus a generous 10s backstop (measured ~300ms end to end — the guard catches accidental quadratic blowups, not machine speed). Recovery hardening fixed two real defects: **(1) writeback orphan-lock lockout** — a writer crash after lock creation but before the journal used to leave a permanent `WRITEBACK_ALREADY_RUNNING`; `recoverCapCut81SameProfileWriteback` now clears the stale lock when no journal exists and returns the new action `cleared-stale-lock` (safe because mirror mutation starts only after the journal exists; a journal missing its rollbacks stays a hard `RECOVERY_REQUIRED`, both directions tested); **(2) inbox listing single-point failure** — one corrupt entry used to fail the whole `listDesktopImports` Promise.all; corrupt entries now return as `unreadable: true` placeholders (visible, deletable, never readable/publishable, never silently hidden) and in-flight temp directories never list. Tests: writer +2, inbox +2. **Remaining**: the mid-rename crash matrix (afterMirrorReplaced 1..3 partially covered), stranded-data audit for corrupted renderer ImportJournal records, quarantine policy for a corrupt PersistentImportPlanStore file, cache metrics, and 100 GB-scale media validation |
