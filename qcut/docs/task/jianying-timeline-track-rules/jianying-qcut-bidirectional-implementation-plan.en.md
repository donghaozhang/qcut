...
**Date:** 2026-08-04  
**Last verified revision:** `0f81e11ff`
...
| --- | --- | --- | --- |
| JYI-014N JianYing 11.3 beta4 video X-position linear-keyframe import | ✅ Minimal milestone done | 2026-08-13 | A controlled real-app experiment used a three-second 640×360/30fps video, storing X=0 at 0s and UI X=50 at 2s. The active plaintext subdraft proves that JianYing writes paired `KFTypePositionX` and `KFTypePositionY` `Line` channels, with final X `0.078125 = 50/640` and Y fixed at zero. The beta4 mapper accepts only exactly those two channels with matching ordered timestamps, a zero-time first point, zero controls, empty graph/string values, one finite value per entry, in-segment times, and exact QCut frame-grid alignment; the final `clip.transform` must match the channel endpoint and all existing video/default-companion gates must still pass. This subset becomes QCut canvas-space X=0→25 at frames 0→60 (the persisted `0.078125` is in half-canvas units — the `clip.transform` convention — so JianYing's UI value 50 is a 25 px canvas offset on a 640 px canvas). Unknown curves, nonzero Y, timestamp mismatch, unknown fields, or non-frame-aligned times preserve the source and fail closed to opaque. After rebuilding the import runtime, the production CLI reports one exact segment, one resolved resource, zero issues, and `canCommit=true` for the real draft; 17 focused tests and repository-wide type checking pass. This milestone does not claim Y motion, scale, rotation, opacity, Bezier curves, keyframe writeback, JianYing save/reopen, or frame-level render parity; path-bearing evidence remains outside Git. |
| JYI-014L JianYing 11.3 beta4 static-audio import | ✅ Minimal milestone done | 2026-08-13 | A real-app-created three-second standalone WAV compound subdraft now passes the production import path. The root is unwrapped only when it is a single `audio` wrapper whose nested draft contains a resolvable audio segment; the nested empty `mixed` main track is retained as an empty QCut main video track. Audio is exact only for local `extract_music`, matching full-length source/target ranges, unit speed/volume, no loop/reverse/tone modification/intensification/keyframes, and the real-fixture default values in all five `beats`, `placeholder_infos`, `sound_channel_mappings`, `speeds`, and `vocal_separations` companions. Any volume, playback, or companion-processing change remains opaque instead of being approximated. Real CLI `inspect/plan` reports two tracks, one segment, one resolved audio resource, zero issues, and `canCommit=true`; `verify-roundtrip` proves byte-identical no-op preservation of the 16,802-byte active subdraft. After an isolated live QCut import, trusted renderer double-read proves 1920×1080/30fps, the empty main track, one audio track, one three-second media element, and 288,078 persisted audio bytes. This milestone claims import and no-op preservation only for that narrow subset; it does not claim volume/speed/audio-processing mapping, audio writeback, opaque-parent ownership, or JianYing save/reopen. `writebackPerformed=false` and `targetAppPersistenceVerified=false`; path-bearing drafts, media, and live receipts stay outside Git. |
| JYI-014M JianYing 11.3 beta4 adjacent-video import | ✅ Minimal milestone done | 2026-08-13 | A clean real-app project placed two QCut-owned three-second H.264 clips adjacently on the main timeline and converted them into one compound clip. Its plaintext nested draft proves one `mixed` track with two ordered segments at `0–3s` and `3–6s`, two local video materials, and six default companion references per segment. The beta4 mapper marks a video exact only when it is a full-length local clip with matching source/target duration, track/render indices matching source-track order, default crop/transform/playback/visibility/color/algorithm/mask state, and verified default `speeds`, `placeholder_infos`, `canvases`, `sound_channel_mappings`, `material_colors`, and `vocal_separations`; any changed trim, crop, transform, playback, keyframe, algorithm, mask, color, or companion processing fails closed to opaque. Real CLI `inspect/plan` reports one track, two segments, two resolved resources, zero issues, and `canCommit=true`; no-op verification is byte-identical. An isolated live QCut import then persisted 640×360/30fps, one media track, the same blue→red order and timing, and two video payloads whose byte lengths and SHA-256 values match their sources. A trusted renderer double-read captured that state without absolute paths. This milestone does not claim transformed/cropped video mapping, frame-exact JianYing export, writeback, opaque-parent ownership, or JianYing save/reopen; private drafts, media, and receipts remain outside Git. |
...
| JYI-003 Profile registry/detection | ✅ Done | 2026-08-13 | `jianying-draft/profiles/` + `import/profile-detection.ts`. Profile contracts grade each operation as none/fixture/candidate/stable; `isDraftProfileWritable` checks only `sameProfileWriteback: stable`, so stable inspect/import never accidentally enables writeback. The registry contains synthetic 5.9, CapCut 8.1, and JianYing 11.3 beta2/3/4; beta2/3 remain non-production candidates, beta4 promotes only the real-E2E-covered inspect/import to stable/production, and every JianYing 11.3 write capability stays none. Detection uses four independent signals (app metadata, schema/new_version, top-level key containment, and file layout); missing exact app metadata yields `ambiguous`, which never writes; encrypted content is terminal; a file name alone decides nothing. Tests: [`profile-detection.test.ts`](../../../packages/editor-core/src/__tests__/profile-detection.test.ts) cover registry state, exact beta2/3/4 detection, unknown versions, ambiguity, unsupported, encrypted, and the writeback gate. |
...
| JYI-014J JianYing 11.3 beta4 minimal production import | ✅ Done | 2026-08-13 | A live CLI import consumed the only plaintext compound subdraft in a real beta4 sacrificial project after explicit acceptance of two opaque warnings. A fresh isolated QCut profile persisted a 1920×1080/30fps project with one video resource and one media track; the element retained `startTime=0.4s` and `duration=8s`. `qcut editor interop import-snapshot` then double-read renderer storage, revalidated the 4,511,472-byte media length and SHA-256, and emitted no absolute path; both the active subdraft and opaque parent hashes stayed unchanged across import. beta4 `inspect/import` is therefore stable/production only for this core single-video compound subset. JYR-007 proves writeback ownership fails, so `sameProfileWriteback` and `crossProfileExport` remain `none`, `realAppVerified` remains false, and a real `qcut draft export` is blocked as `profile-not-writable`. Private drafts and receipts remain local and outside Git. |
| JYI-014K JianYing 11.3 beta4 static-text import | ✅ Minimal milestone done | 2026-08-13 | A real-app-created single plain-text compound subdraft now passes the same production import path. A pure-text `mixed` track becomes text; one full UTF-16 style, unit transform, transparent background, omitted stroke/shadow arrays, and a proven empty `material_animations` companion map to a QCut TextElement. Effective animations, decorative/text effects, keyframes, and multiple styles remain opaque/blocked. The real CLI plan is exact with `canCommit=true`, zero blockers, and one explicit downgrade. Both offline commit and live import refuse missing warning acceptance; exact acceptance atomically queues an isolated inbox entry and also imports into a matching running QCut. Trusted renderer double-read proves 1920×1080/30fps, one text track, one three-second transparent-background text element, zero media, and zero skipped nodes. `qcut draft verify-roundtrip` also proves byte-identical no-op preservation of the active creation-time subdraft while explicitly reporting `writebackPerformed=false` and `targetAppPersistenceVerified=false`. Real-app behavior further proves compound subdrafts are creation snapshots: later nested edits do not update old snapshots, and old/new snapshots coexist, so root discovery with multiple candidates must require explicit selection rather than guessing an active owner. This milestone claims warning-gated static-text import only, not frame-exact font rendering, text writeback, or opaque-parent ownership; private drafts, screenshots, and path-bearing evidence stay local and outside Git. |
| JYI-015 Same-profile envelope/writeback | 🟨 Partial | 2026-08-05 | Imported projects persist a versioned source binding, normalized `DraftInteropDocumentV1` baseline, and optional `ForeignDraftEnvelope` metadata; raw source bytes remain only in the OS-keychain-wrapped encrypted payload. A receipt-bound real CapCut 8.1.1 saved draft (`new_version` observed at `179.0.0`) captures only `draft_info.json` from 25 discovered files: 14,273 source bytes, 6 bindings, a 19,115-byte payload, and zero warnings. Evidence: [`capcut-8.1.1-envelope-capture-2026-08-04.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-envelope-capture-2026-08-04.json). The writeback chain includes strict payload/key-version/SHA-256 verification, scalar JSON Pointer patches with old-value and unknown-ownership conflict guards, the core video/audio timing planner, unknown-preserving preparation, renderer orchestration, and trusted Electron IPC bound to a short-lived directory-selection token. The planner consumes a dedicated pathless timing snapshot (tracks plus playback-aware durations), so imported IndexedDB media never needs an export-only filesystem path. The main-process writer transactionally replaces exactly four active mirrors, never touches `.bak`, and provides fsync, independent rollback copies, journal recovery, `.locked`, symlink, TOCTOU, and source-hash gates; add/delete/cross-track moves, resource replacement, and speed changes fail closed. The receipt-bound isolated-copy verifier injected a controlled unknown segment sentinel, ran production `inspect → plan → commit`, applied four timing leaves, and proved all four active mirrors matched, both `.bak` mirrors and the original source stayed unchanged, only the planned JSON pointers changed, the sentinel survived, and recovery returned `none`. Path-free evidence: [`capcut-8.1.1-same-profile-writeback-2026-08-05.json`](../../../scripts/capcut-e2e/receipts/capcut-8.1.1-same-profile-writeback-2026-08-05.json). User delivery is now wired end to end: an export-dialog card exposes writeback/recovery states for imported CapCut 8.1 projects; the strict wire API accepts only `projectId` or an opaque `recoveryToken` and rejects draft paths/unknown keys; utility HTTP → main → trusted-main-frame renderer IPC reloads persisted project/timeline state, correlates operation/project identity, and revalidates the captured timeline after directory selection so concurrent edits fail as `qcut-state-changed`. CLI commands are `qcut editor interop writeback --project-id <id> --json` and `qcut editor interop writeback-recover --recovery-token <token> --json`. Focused validation passed 74 tests across 13 files, Web/Electron TypeScript checks, both production builds, and a live CLI→utility→main→renderer blocked-path smoke test. Writeback verification now accepts an optional `--app-receipt` and emits derived, path-free schema-v2 evidence. The receipt must bind exact 8.1.1 app metadata, the approved Developer ID signature, distinct open/reopen process generations, the pre-open writeback SHA-256, four matching active mirrors, the unknown sentinel, and byte-identical saved/reopened mirror state; 9.1, a wrong case/profile/hash, missing phases, or reopen drift fails closed. A dedicated receipt producer now records `pre-open → opened → saved/quit → reopened → final quit` through a strict CLI. Every boundary revalidates the dedicated account, disposable-store registration, immutable draft directory identity, exact app/signature, and expected process state; final path-free plan/result/receipt files are mutually SHA-256-bound, while the local path-bearing state remains excluded. A bare receipt, changed companion, mirror path escape, reused process generation, or replaced draft directory fails closed; focused producer/consumer coverage is 16 tests. Historical receipts are not rewritten, the locally installed app is 9.1.0, and no real 8.1 app receipt exists yet, so the verdict intentionally remains `unverified` and the profile remains `sameProfileWriteback: none`. |
| JYI-016 feature mappers | 🟨 Partial | 2026-08-13 | Four fail-closed import slices are implemented. First, evidence-gated CapCut 8.1 native `Dissolve` maps to QCut `dissolve/easeInOut` only after exact identity, adjacent seam, positive integer duration, and adjacent-segment capacity checks. Second, a shared static single-style plain-text mapper accepts only one style covering the full UTF-16 range, finite canvas/transform values, unit scale, no flip, and no keyframe, effective animation, or decorative effect. JianYing beta4 additionally permits only its real-fixture-proven empty `material_animations` companion and omitted stroke/shadow arrays. It maps content, family, size, fill, alignment, weight/style/underline, spacing/width, position/rotation/opacity, one stroke, one shadow, and background. Multi-style, subtitle semantics, dynamic/decorative text, and malformed values are blocked/opaque rather than silently flattened. Third, a JianYing-beta4-only static local-audio mapper marks exact only a full-length `extract_music` clip with default playback/volume state and all five real-fixture default companions; trim, speed, volume, loop, reverse, tone, vocal/channel processing, and keyframes fail closed to opaque. Fourth, a JianYing-beta4-only static local-video mapper marks exact only full-length clips whose track/render indices agree with source-track order and whose material, segment, and six companion objects match real-app defaults; trim, crop, transform, playback, visibility, color, keyframes, algorithms, masks, and companion processing fail closed to opaque. CapCut 8.1 text remains downgrade because no real render receipt exists. JianYing beta4 text remains downgrade because system-font substitution and stale creation-time subdraft ownership can differ from the live project; neither text profile is claimed frame-exact. JianYing beta4 audio/video exactness likewise means exact structural and persisted-state mapping, not spectrum- or frame-verified JianYing export. The bundle parser enforces track/element agreement and deterministic projection, while the renderer persists zero-media text, audio payloads, and ordered multi-video timelines. Focused mapper/session regression, repository-wide `check-types`, and the Electron/import-runtime build must remain green. Color, mask, keyframe, rich/dynamic text, audio processing, transformed/cropped video, and proprietary-transition mappers remain. |
...
| JYI-018 Scale/recovery hardening | 🟨 Partial | 2026-08-05 | Scale: [`draft-interop-scale.test.ts`](../../../packages/editor-core/src/__tests__/draft-interop-scale.test.ts) runs 10k segments (10 tracks × 1000 + 20k materials) through the full pure pipeline read→validate→normalize→map→diff→bundle-parse with zero issues and zero RESTRICTED path leakage; per-stage count assertions plus a generous 10s backstop (measured ~300ms end to end — the guard catches accidental quadratic blowups, not machine speed). Recovery hardening fixed six real defects: **(1) writeback orphan-lock lockout** — a writer crash after lock creation but before the journal used to leave a permanent `WRITEBACK_ALREADY_RUNNING`; `recoverCapCut81SameProfileWriteback` now clears the stale lock when no journal exists and returns the new action `cleared-stale-lock` (safe because mirror mutation starts only after the journal exists; a journal with neither a valid rollback nor an already-restored active mirror remains a hard `RECOVERY_REQUIRED`); **(2) inbox listing single-point failure** — one corrupt entry used to fail the whole `listDesktopImports` Promise.all; corrupt entries now return as `unreadable: true` placeholders (visible, deletable, never readable/publishable, never silently hidden) and in-flight temp directories never list; **(3) corrupt persistent-plan startup lockout** — malformed regular-file state is now renamed into a mode-0600 quarantine inside the private plan directory, all untrusted old tokens are invalidated, and the empty store resumes with a path-free `startupRecoveryAction`; symlink and non-file store paths remain hard unavailable and are never quarantined; **(4) renderer journal misattribution** — recovery previously trusted any persisted object with `schemaVersion: 1`, so a corrupt storage-key/`importId` pair could authorize cleanup using an unrelated `projectId`. A dedicated exact parser now validates key identity (including IndexedDB's `id`), lower-case bundle digest, phase, bounded safe IDs, unique media IDs, and canonical monotonic timestamps. `ImportJournal.audit()` admits only parsed copies; unreadable/corrupt records remain untouched, contribute only a count, and never invoke storage deletion. The import UI displays that count without exposing tokens or keys and states that no project data was deleted from those records. **(5) non-idempotent writeback rollback** — recovery renamed rollback files over the four active mirrors, so a second crash after consuming one to three rollback files made the next startup reject the journal permanently. Recovery now preflights every mirror before mutation, accepting only a rollback file whose SHA-256 matches `expectedSourceSha256`, or an already-restored active mirror with the same digest; if any mirror has neither, it fails before touching the others. A fault matrix covers writer crashes after 0–4 active replacements, recovery crashes after 1–4 rollback renames, final digest/`.bak`/artifact invariants, a second no-op recovery, and the irrecoverable fail-before-mutation case. **(6) corrupt-journal operator dead end** — records whose ownership cannot be proven can now be explicitly isolated from automatic recovery without deleting or trusting their source data. A separate quarantine namespace stores only a bounded SHA-256 fingerprint of the source key plus raw JSON value and a canonical timestamp; it stores neither the key nor the value. `ImportJournal.audit()` suppresses a corrupt record only while its current fingerprint matches a strict read-back-verified marker. Source mutation, malformed markers, unavailable Web Crypto, oversized input, or marker-write failure all fail back to an active corruption warning. The UI exposes counts and an explicit isolate action but never identifiers or fingerprints; project, timeline, media, and source-journal deletion are absent from the operation. A bounded per-resolution asset work cache now deduplicates concurrent file hashes and bounded name searches across repeated resource references, caps entries with LRU eviction, rejects files whose identity changes during hashing, and reports schema-versioned path-free hit/miss/eviction/hashed-byte metrics through plan JSON, CLI, and IPC. This is deliberately an ephemeral work cache, not a claim that persistent content-addressed derived caches are complete. A shared schema-versioned recorder now reports only stage IDs, measured durations, and invocation counts. Runtime planning covers request validation, discovery, snapshot read, profile detection, normalization, asset resolution, timeline mapping, bundle validation, and plan persistence; these path-free metrics survive plan JSON, CLI, and IPC. The renderer transaction separately measures bundle/digest/envelope validation, quota and identity checks, journal start, media/timeline staging, read-back verification, envelope persistence, project publication, and every attempted rollback. Large-media groundwork now shares an 8192-entry bound between discovery and filename indexing, excludes asset bytes from control snapshots, and verifies that a logical 100 GB/5000-asset inventory consumes none of the 256 MiB control budget. Resolved SHA-256/byte length/status evidence is bound into the plan fingerprint; distinct filename lookups share one directory index; 5000 references to one asset perform one hash; commit payload reads recheck `O_NOFOLLOW`, file identity, length, and SHA-256 to close the resolver-to-payload TOCTOU window. Import-runtime regression passes 95 tests across 9 files; the shared recorder has 3 focused tests; renderer storage regression passes 44 tests across 8 files; the same-profile export suite passes 133 tests across 14 files; focused UI/hook suites pass 8 cases; repository-wide `check-types` and the Electron production build pass. Large-media transport now removes the whole-file Base64 hot path from live/CLI imports: the main process issues path-free, capacity-bounded grants with a two-hour inactivity timeout refreshed by successful reads, rechecks file identity on every bounded offset read, and the renderer validates token/length/EOF before writing 4 MiB default chunks directly to OPFS. Persisted length/SHA-256 is reverified, failures roll back, and grants are explicitly released in `finally`. The desktop inbox likewise streams with bounded concurrency, hashes while writing, and atomically publishes by temporary-directory rename. Sixty-eight focused transport/storage tests and repository-wide type checking pass. **Remaining**: persistent/cross-session content-addressed caches, calibrated per-stage budgets with production aggregation/alerting, and a real 100 GB media end-to-end import receipt; the existing 100 GB result remains inventory-level coverage. |
...
| JYR-007 compound-draft ownership | 🟥 Writeback blocker | 2026-08-13 | In a JianYing macOS 11.3.0-beta4 single-variable video experiment, only nested `target_timerange.start` in the active plaintext `subdraft/draft_content.json` changed from 400000 to 1000000 microseconds. The compound UI initially showed the one-second gap, but switching away and saving caused JianYing to rewrite the opaque parent and active subdraft and restore 400000. A text experiment then proved the subdraft is a creation snapshot: editing plain text inside the compound survived save/reopen in the UI and opaque parent, while the old plaintext subdraft content and mtime did not change; breaking apart and creating a new compound added another snapshot while the old one remained. Therefore a plaintext subdraft is neither an independently committable final owner nor a reliably discoverable active owner from root candidates. “Patch plaintext and let JianYing adopt it” fails; 11.3 must keep `sameProfileWriteback: none`, and multi-candidate imports require explicit selection. |
...
> In JYI-016, “keyframes remain” now means keyframe shapes and writeback outside JYI-014N; the verified linear X-position subset is no longer part of that gap.
...
- Complex text, color, masks, keyframes outside JYI-014N, and transitions still produce many blocking or warning mappings.
...
```bash
qcut draft import --format jianying --draft "/path/to/draft-folder" --json
qcut draft export --format jianying --project-id <qcut-project-id> --json
qcut draft verify-roundtrip --format jianying --draft "/path/to/draft-folder" --json
```

The current `verify-roundtrip` success scope is only `active-subdraft-noop`: it proves the import projection can regenerate a byte-identical active plaintext subdraft when the QCut timeline is unmodified, and its result must report both `writebackPerformed: false` and `targetAppPersistenceVerified: false`. It is not equivalent to a real JianYing save/reopen and does not lift JYR-007. `export` must fail with `profile-not-writable` for any profile that has not reached stable writeback; it may not choose a “closest version” or bypass the gate.
...
| JianYing 11.3.0-beta4 plaintext compound subdraft | Stable inspect | Core single-video subset stable | Blocked | Blocked | Import real-app verified; opaque parent ownership unresolved |
| Other JianYing 11.x newer formats | Read-only first | Blocked without evidence | Blocked | Blocked | Unresolved/encrypted |
