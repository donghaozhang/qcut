...
wzrdagentstudio 新加一条路由，浏览器里开一个活的 `qcut` 终端。用户点 dashboard 卡片，React 组件调 `/sandbox-spawn`、拿到 `ws_url`，把 xterm.js 接到 WebSocket 上，用户边敲 `qcut …` 边看输出回流。

这是 PR 06-08 所有后端的**消费端**。

## 依赖

- PR 07 —— spawn 端点在、返回 `ws_url`。
- PR 08 —— 中继可达、转 PTY 字节。

> **仓库**：`/Users/peter/Desktop/code/wzrdagentstudio/`（不是 `/Users/peter/Desktop/code/qcut/qcut/`）。下面所有路径在 **wzrdagentstudio**。

## 涉及文件

| 路径 | 动作 | 用途 |
|------|------|------|
| `src/features/qcut-sandbox/routes/SandboxRoute.tsx` | 新 | 路由（`/sandbox/:workspaceId`） |
| `src/features/qcut-sandbox/components/TerminalView.tsx` | 新 | xterm.js 挂载 + WS 管道 |
| `src/features/qcut-sandbox/components/SessionHeader.tsx` | 新 | TTL 倒计时 + Stop 按钮 |
| `src/features/qcut-sandbox/components/ResourceClassPicker.tsx` | 新 | standard/large 下拉 |
| `src/features/qcut-sandbox/hooks/useSpawnSandbox.ts` | 新 | 调 `/sandbox-spawn` |
| `src/features/qcut-sandbox/hooks/useSandboxSocket.ts` | 新 | WS 生命周期 + 重连/idle UX |
| `src/features/qcut-sandbox/api/sandbox-client.ts` | 新 | 类型化 fetch |
| `src/integrations/supabase/functions.ts` | 改 | 注册 `sandbox-spawn` 函数名 |
| `package.json` | 改 | 加 `@xterm/xterm`、`@xterm/addon-fit`、`@xterm/addon-web-links` |
| `src/router.tsx`（或等价） | 改 | 挂上新路由 |
...
export function SessionHeader({ sessionId, expiresAt, onStop }: Props) {
  ...
  return (
    <div className="flex items-center justify-between border-b px-3 py-2 text-sm">
      ...
      <button className="rounded bg-red-600 px-2 py-1 text-white hover:bg-red-700" onClick={onStop}>Stop</button>
    </div>
  );
}
```
...
```tsx
interface Props { value: "standard" | "large"; onChange: (v: "standard" | "large") => void; }
export function ResourceClassPicker({ value, onChange }: Props) {
  return (
    <select className="rounded border px-3 py-1" value={value}
...
在现有 dashboard 组件加 "qcut Shell" 卡 / 侧栏项，跳 `/sandbox/<当前 workspace id>`。如果项目有 feature flag 系统就用 `qcut_sandbox_enabled` 把门；没有就不门。
...
vi.mock("@xterm/xterm", () => ({
  Terminal: vi.fn().mockImplementation(() => ({
    ...
    dispose: vi.fn(), cols: 80, rows: 24,
  })),
}));

describe("TerminalView", () => {
  it("挂载时对给定 wsUrl 开 WS", () => {
    const wsMock = vi.fn();
...
跑：`bun --cwd /Users/peter/Desktop/code/wzrdagentstudio test`。
...
# 浏览器：登录、走 /sandbox/<workspace-id>
# 点 "Open qcut shell"
# 期望：motd 出现，可以敲 `qcut --help`，能看到输出
# 点 "Stop"——终端关、DB 里 session 行变 `ended/user_kill`
```

浏览器 console 看有没有未处理 WS error。spawn POST 已 200 但终端死黑 = 怀疑中继（PR 08），不怀疑 UI。

## 不在本 PR 范围

- 30 s 重连宽限 UX。v0：连接掉 = 用户重点 "Open qcut shell"。
- 历史会话视图。容易加，本 PR 不做。
- 拖拽上传文件进沙箱。Phase 3。
- 用户自定义主题。用 xterm.js 默认 + 项目 tailwind tokens。
- 移动端布局。终端在手机上是反特性——desktop only。

## 相关文档

- [`../web-sandbox/integration.md`](../web-sandbox/integration.md) —— 文件布局背景 + 变体
- [`07-spawn-edge-function.md`](07-spawn-edge-function.md) —— 调的后端
- [`08-relay-worker.md`](08-relay-worker.md) —— 连的 WS 对端
