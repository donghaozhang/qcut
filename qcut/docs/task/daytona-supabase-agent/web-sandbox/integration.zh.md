...
终端在 React 应用里住哪、Supabase 那侧加什么 Edge Function、两边契约长啥样。配 [`web-sandbox-architecture.zh.md`](architecture.zh.md) 读。
...
- [`web-sandbox-verification.zh.md`](verification.zh.md) —— 接好之后怎么确认能工作
- [`web-sandbox-architecture.zh.md`](architecture.zh.md) —— 组件拆解
- [`container-setup.md`](../core-plan/container-setup.md) —— 镜像结构（`qcut-entrypoint.sh` 放这）
- [`secrets-supabase.md`](../core-plan/secrets-supabase.md) —— 上面引到的 `agent_secrets` schema
