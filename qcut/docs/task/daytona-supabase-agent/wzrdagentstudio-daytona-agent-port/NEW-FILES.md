...
- Store Daytona runtime sessions separately from the existing logical `wzrd_agent_sessions`.
...
3. RLS policies
   ...
   - authenticated users can insert/update only their own sessions
...
- Centralize env names, defaults, paths, and token settings.
...
- Sign short-lived JWTs consumed by the Cloudflare relay.
...
- Keep DB file metadata optional for the first release.
...
- Issue a short-lived relay WebSocket token.
...
- Configure Worker, Durable Object, and env vars.
...
- Upload source files into the sandbox input dir.
...
If project-level context matters, use:
