...
1. Confirm whether WZRD needs a project-scoped agent:
   - global route: `/agent`
   - project route: `/projects/:projectId/agent`
2. Confirm Daytona image name:
   - env: `DAYTONA_AGENT_IMAGE`
3. Confirm relay deployment URL:
   - env: `DAYTONA_RELAY_URL`
4. Confirm token secret:
   - Supabase functions and Cloudflare relay need the same `DAYTONA_RELAY_JWT_SECRET`

## Phase 1: Database

1. Add `create_daytona_agent_runtime.sql` migration.
2. Create tables:
   - `daytona_agent_sessions`
   - `daytona_agent_events`
3. Add RLS, indexes, and an `updated_at` trigger.
4. Run the migration locally or in staging.
5. Update generated Supabase types if WZRD uses generated DB types.
