...
| L3 变速标量 → playbackRate | ✅ 2026-08-19 landed。指纹放宽:常速 companion(`isConstantRateSpeed`,须与段 speed 一致)+ 时序关系校验(target = source ÷ speed,±1µs 容差)+ 曲线变速/不一致维持 opaque。导出侧:segment `duration` 定为时间线秒 + `playbackRate` 字段,视频 `setpts=(PTS-STARTPTS)/rate` + **必须链内 `fps=` 阶段**(仅靠输出 `-r` 会把 90 帧错采成 47 帧/斜率 1.914,链内 fps 精确 2×,实测 45帧/1.500s/逐帧 2N 映射),音频 atempo 链([0.5,2] 分段)。QCut 侧自检通过(on 第 N 帧 = off 第 2N 帧);**剪映 ground truth 待下个副屏窗口采集**(speed + scale/alpha/position 四案一起)。 |
| L4-L8 | 未开始(规划) |
