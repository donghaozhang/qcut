...
| L2 变换标量 → transform | ✅ 2026-08-19 landed + **首份真机 pass 回执**。指纹放宽(`readBeta4ClipTransform`:alpha/rotation/统一 scale/静态位移)、visual 走 QCut 约定(半画布单位×canvas/2,旋转同号)、plan/物化/bundle 全链透传。**顺带修了 QCut 全局导出 bug**:媒体元素 transform 在所有导出管线都被丢弃(时间线快照 media 分支漏序列化 + 主进程分段管线无 transform 滤镜),已修快照序列化 + 主进程 ffmpeg 变换链(scale→rotate→pad/crop→opacity)。transform-rotation 平价:isolation 128.6 / parity 16.0 / cross 128.6 / baseline 8.5 → **pass**(比对器统一 bt709 解码假设 + 相对阈值:残差 ≤ 底噪+15%×特征强度)。**渲染器引擎缺陷已修(2026-08-19 同日)**:新增 `export-engine-utils.drawWithMediaTransform`(以 bounds 中心为锚:translate(center+x/y)→rotate(顺时针度)→scale(flip 走负号)→globalAlpha×opacity,恒等变换零开销跳过,与预览 CSS 语义逐项对齐并有调用序列单测);Standard/CLI/Muxer 共用 `export-engine-renderer` 的 renderImage/renderVideo(包一处全覆盖,drawColorGradedSourceWithMasks 落画布走 drawImage 吃 ctx 变换),Optimized 三处裸 drawImage 单独接线(含 keyframes 解析)。导出侧 174 测试全绿。 |
...
| L7 特效段 → jianying-local | ✅ 2026-08-19 landed(draft-profile 分支)+ 端到端真机链路验证。新增 `import/beta4-effect-mapper.ts`:**本机可用性条件准入**——`normalizeRawDraft` 新增 `localJianyingEffects` 注入口(editor-core 保持纯净,不碰文件系统),导入宿主链:electron `jianying-draft-import-handler` 用 `discoverJianyingEffectLibrary()` 过滤 **installed && supported**(本机 617 个)→ session 每会话缓存一次 → normalize。命中(resource_id 在表 + name 与本机目录一致)→ 特效段 opaque **升为声明式 downgrade**(`jianying-local-effect:<presetId>`,证据注明本机运行时渲染 + 批渲验证 + **导出阻塞属预期**);未装/未验证/名字不符/无注入 → 维持 opaque(机器诚实:同一草稿换机器结论不同,by design)。plan 新增 effect 轨/元素(presetId/packageHash/adjustParameters),物化为 `EffectElement`(engine jianying-local,滑杆置包默认值,effectType "brightness" 占位与实验室约定一致)。**端到端**:fixture 草稿(70s 特效 1s-3s)导入 → 指纹门 → 时间线 effect 轨元素 `{engine: jianying-local, packageHash ✓, adjustValues 包默认}`。**边界注记**:滑杆值导入走包默认(段级 adjust 值的草稿形状无明文样本,待证据);渲染保真由特效实验室批渲参照线背书。854 测试全绿。 |
| L8 贴纸段 → 参照资产导入 | ✅ 2026-08-20 landed(draft-profile 分支)+ 端到端真机链路验证。新增 `import/beta4-sticker-mapper.ts`:**可用性条件 = 草稿内嵌图片资产本身**——贴纸材料带可 staging 的本地图片 `path`(png/gif/webp/jpg)即准入为声明式 downgrade(`jianying-reference-sticker:<resource_id>`),资产复用媒体 staging 管线进用户本地项目存储;无资产/非图片格式 → 维持无声明 downgrade 被 skip(贴纸 kind 基线本就是 downgrade,L0 声明门天然适配)。**Provenance 纪律**:声明的 fidelityEvidence 全程标注「剪映参照 · 内部 — never redistributed」,资产只进本地项目、不进 Git、不进公开渠道(与贴纸实验室私有目录同界)。plan 新增 sticker 轨/元素(resourceId→staged mediaId),物化为 `StickerElement`(overlay 默认:居中 50/50、15% 尺寸;段级 transform 留待明文样本)。**注**:原计划设想对表 `local-sticker-manifest.ts` 私有目录,实测该 manifest 无 resource_id 字段且资产在 Supabase 白名单侧——改为草稿内嵌资产路线,语义更直接且离线可用。**端到端**:fixture 草稿(参照贴纸 PNG,1s-4s)导入 → 指纹门 → sticker 轨元素 `{mediaId staged ✓, x/y 50/50, size 15%}`。857 测试全绿。**L0-L8 全线完成。** |
