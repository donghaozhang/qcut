...
新增默认关闭的 Electron 本地“音效实验室”,用于浏览和验证 382 条剪映参照
音效。它支持 20 分类、搜索、真实试听和加入时间线,但不会提交/上传剪映音频,
也不会把临时本地 URL 写入收藏或最近使用。实现、启用命令、库存、测试、失败
修复和下一步见 [Sound Effects Lab](../sound-effects-lab/README.md)。
