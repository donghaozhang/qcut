...
首批原生 UI 逐项对应的 8 套模板、69 个去重文件、33,938,996 字节，已复制并校验：
...
**8 套是已观察、已下载的子集，不是在线全量。** 当前 1 套的显式依赖齐全，7 套仍有缺失或旧路径；8 套均标记原生渲染未接入。点击卡片查看缓存详情，不会用缩略图或原创预设冒充模板套用。文件存储由 QCut 管理不意味着取得素材再分发权；这些字节不进 Git、不打包发布。

导入、分批追加、校验与恢复命令见 [缓存实测记录](./native-cache-2026-09-06.zh-CN.md)。阻止读取整个剪映目录后，SSD 备份仍独立通过全部 69 个文件校验。
