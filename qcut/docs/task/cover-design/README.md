...
更新：2026-09-05。状态：第一阶段可用，完整封面模板编辑器尚未实现。
...
1. 预览面板工具栏增加“项目封面”入口，支持鼠标、键盘和中英界面。
2. 图片导入接受 PNG/JPEG/WebP，解码后统一成 PNG；使用项目画布尺寸，支持完整显示和居中填满。
3. 当前帧复用正式静帧导出渲染器，不截取编辑器界面；保存场景、帧号、FPS 和时间来源。
4. 媒体仍在加载或当前帧的可见媒体缺少 URL 时拒绝取图；继续执行已有受限素材导出检查。
5. 独立保存单图层 `CoverDesignV1`、项目尺寸 PNG、640×360 WebP 缩略图，以及 `project.cover` 引用。
...
10. 封面弹窗在 1440×1000 和 390×844 下验证；修复时间线层级遮挡，未改其他弹窗的默认层级。
...
边界限制：单张输入不超过 32 MiB，单边不超过 8192，像素总数不超过 33,554,432；本轮模型只接受一个图片图层，不能把未知文字、贴纸等图层悄悄丢掉后声称成功。
...
- 封面编辑器文字、图层、裁剪移动、字体、描边、阴影，以及撤销重做；模板卡片须使用有授权的 QCut 自有资源。
- 模板 schema、参数绑定、缩略图生成、离线包和缺依赖错误；剪映目录卡片与完整可运行包分开统计。
- Electron 原生文件系统存储、项目包打包与重开、垃圾回收、并发写入事务及跨进程锁。
- CLI/API 的创建、编辑、渲染和封面绑定接口。
- 接通现有主轨道上的封面占位标记；本轮入口位于预览面板工具栏。
- 原生 Electron 实机 E2E、视频/复杂多图层/转场场景的逐像素验证。当前静帧渲染路径仍有原有转场边界差异，不能据此宣称剪映完整视觉对齐。
