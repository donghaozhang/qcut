...
更新：2026-09-06。状态：可编辑原创模板工作区，以及剪映八分类和首批私有资源缓存；尚非完整素材库或原生模板渲染兼容。
...
**8 套是已观察、已下载的子集，不是在线全量。** 当前 3 套的显式依赖已有文件副本，5 套各缺一个旧滤镜。复用现有花字/字体/滤镜实验室，新增精确 hash 恢复、目录 ID 别名、版本映射和有版本约束的内置资源绑定；原始定义不改写，映射来源在详情中显示。5 个旧滤镜 ID 在当前及已检查的 36 份历史滤镜数据库中未找到，不能用同名资源冒充。8 套均标记原生渲染未接入。点击卡片只查看缓存详情；这些私有字节不进 Git、不打包发布。
...
- 气泡素材、自定义字体和复杂原生花字；多选、直接缩放/旋转手柄和更完整的裁剪比例交互。静态描边/阴影/背景/发光参数已补齐，但不代表原生花字渲染兼容。
