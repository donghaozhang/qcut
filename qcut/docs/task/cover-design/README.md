...
- [中文：同素材视觉对照、竖屏映射与原生花字颜色](./visual-parity-2026-09-06.zh-CN.md)
- [English: same-source visual parity, portrait mapping and native text colors](./visual-parity-2026-09-06.en.md)
- [中文：Iceland 侧转文字、画布空间与保存重开](./sideways-text-2026-09-06.zh-CN.md)
- [English: Iceland sideways text, preview space and save/reopen](./sideways-text-2026-09-06.en.md)

## 已实现

1. 预览面板工具栏和主轨道“封面”按钮打开同一个编辑器，支持鼠标、键盘和中英界面。
2. 图片导入接受 PNG/JPEG/WebP，解码后统一成 PNG；使用项目画布尺寸，支持完整显示和居中填满。
3. 当前帧和指定帧复用正式静帧导出渲染器，不截取编辑器界面、不移动时间线播放头；保存场景、帧号、FPS 和时间来源。底部支持导入、逐帧、帧号、滑杆和默认展开的十点胶片缩略图；等待时间线和媒体恢复，保留用户主动收起的选择。
...
15. 新增“套用文字布局”：八个真实缓存样本中七个通过资源准备；周末、S23、HERO 完成实际出图和保存重开，HERO 可改为 QCUT 并保留原生效果。依赖状态区分缺背景滤镜、文字资源就绪和竖排不支持；复用自有字体与花字缓存，不替换用户背景。该历史增量 281 项相关测试通过、1 项环境门控测试跳过；五个旧背景滤镜及完整合成仍有缺口。后续已补 TextStyle/InfoSticker 模板颜色覆盖、单击卡片套用、竖屏归一化位置与短边字号缩放，详见同素材视觉对照记录。
16. 第三轮支持经实机确认的纯 ASCII 侧转文字，Iceland 三层文字可编辑套用、改字并保存重开；八个样本现为 8 个文字布局可套用、4 个有实际渲染与保存重开证据。宽高旋转收进图标弹层，1136×676 选中文字时画布高度从 169px 提高至 280px。222 项相关回归通过；中文/混排/原生花字竖排、字号字距精确匹配与完整背景合成仍不属于已完成范围。
...
**8 套是已观察、已下载的子集，不是在线全量。** 当前 3 套的显式依赖已有文件副本，5 套各缺一个旧滤镜。复用现有花字/字体/滤镜实验室，新增精确 hash 恢复、目录 ID 别名、版本映射和有版本约束的内置资源绑定；原始定义不改写，映射来源在详情中显示。5 个旧滤镜 ID 在当前及已检查的 36 份历史滤镜数据库中未找到，不能用同名资源冒充。当前 8 套通过文字布局准备，4 套已有实际渲染与保存重开证据；完整模板背景合成仍未接入。这些私有字节不进 Git、不打包发布。分批采集和四阶段统计见采集记录及第三轮验证记录；本轮已能操作原生封面子窗口，但逐分类全量分页枚举尚未完成。
