...
- **618 个 blit-only 特效全量跑完(含一轮重试):517 成功(84%)**,
  画面特效 490 + 人物特效 27;渲染总时长约 2 小时(均值 12.6s/个,含
  下载),新下载 ~510 个包。参照库 2.3GB + 包 1.2GB。
- 失败 101 个,**100% 归因于 Lumi 家族**(90 个 JS 版:根目录
  `LumiManager.js` + `config.json.js_path`,内嵌 ThreeJS;其余 Lua 版
  `LumiFamily/`)。该家族无 SeekModeScript、墙钟驱动,`effect-video`
  桥接跑完不报帧数。解锁需桥接原生侧支持 Lumi 的逐帧步进——独立后续
  工作。20 个下载网络抖动经重跑全部转为成功。
- SSIM 分布 min 0.048 / p50 0.844 / max 0.988,0 个疑似恒等——所有成功
  参照都有真实画面变化。
- 失败的 Lumi 特效在实验室 UI 中被批渲验证档自动锁定
  (「本机渲染验证未通过」),不会假装可用。
- 坑:桥接"未报帧数"的失败仍会 mux 出一个恒等 mp4,脚本现已在失败时
  自动删除输出(历史遗留的 101 个假参照已手动清理)。
