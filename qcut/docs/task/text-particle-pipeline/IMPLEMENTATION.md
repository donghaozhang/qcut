...
### 1.3b 彩带喷射/福袋炸开实包解剖(2026-07-30 采集,推翻原假设)

- **彩带喷射(入场)= 烘焙 alpha 视频叠加,不是程序化粒子。**
  包 `effect/7563555563871800626`:`res/video_*_alpha.mp4`(1280×640 左右分屏
  alpha,73 帧 3.04s,烟花 starburst 点状射线 + 彩色纸屑)+ `RadialBlur` 节点
  (文字入场径向模糊)。存档:`scratchpad/jy-particles/confetti-burst/`。
- **福袋炸开(循环)= 预渲染 3D 素材叠加**(红福袋 + 金元宝大颗粒,带景深模糊,
  铺满全画布)。应用后零磁盘痕迹(早已缓存),实机播放-暂停帧已存档。
- 结论:QCut 侧用**程序化 burst 近似**(闭式弹道 + spark 射线 + 调色板),
  明确标注为已知差异 —— 不搬运剪映烘焙素材(版权),不引入视频叠加管线
  (导出平价成本过高)。
- 粒子碎落参照帧(75/50/25/0%)确认:溶解前沿是**空间平滑噪声连片推进**
  (白噪声=椒盐,错误),整体自上而下侵蚀、基线残丘最后消失;释放尘埃向上
  飘散并快速衰减 → shatter 用 bilinear value noise + 垂直偏置 + 负重力标定。
