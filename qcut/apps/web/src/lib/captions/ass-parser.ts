...
import { assColorToRgb, assAlignmentToAlign } from "./subtitle-style";
...
function parseEventLine(format: string[], rawValue: string): ASSEvent | null {
	...
	// Split only up to format.length - 1 commas (Text field can contain commas)
	const parts: string[] = [];
	...
	parts.push(remaining.trim()); // Last field (Text) gets the rest
