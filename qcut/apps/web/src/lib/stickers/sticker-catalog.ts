...
export const STICKER_CATEGORY_MINIMUM_SIZE = 30;
...
function themedStickerUrl({
	...
	return `${import.meta.env.BASE_URL}stickers/qcut-original/themed/${category}/${itemId}.svg`;
}
...
function createThemedSticker({
	...
	pack: (typeof THEMED_STICKER_PACKS)[number];
}): StickerCatalogItem {
	const icon = `${pack.id}-${definition.id}`;
	return {
		id: `qcut-themed:${icon}`,
		category: pack.id,
		collection: "qcut-themed",
		icon,
		name: `${pack.name} ${definition.name}`,
		localizedName: `${pack.localizedName}·${definition.localizedName}`,
		tags: [pack.name, pack.localizedName, definition.style, ...definition.tags],
		packId: `qcut-themed-${pack.id}`,
		source: {
			kind: "bundled",
			mimeType: "image/svg+xml",
			url: themedStickerUrl({ category: pack.id, itemId: definition.id }),
		},
		animated: false,
	};
}
...
const THEMED_STICKERS = THEMED_STICKER_PACKS.flatMap((pack) =>
	pack.items.map((definition) => createThemedSticker({ definition, pack }))
);
