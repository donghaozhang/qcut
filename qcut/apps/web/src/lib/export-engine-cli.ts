...
export class CLIExportEngine extends ExportEngine {
  ...
  private async renderMediaElementCLI(
    ...
    try {
      ...
      console.warn(`[CLIExportEngine] Failed to render ${element.id}, using fallback:`, error);
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          console.warn(`[CLIExportEngine] Video seek timeout, using current frame`);
      ...
      console.warn(`[CLIExportEngine] Video render failed, using placeholder:`, error);
  ...
  private getActiveElementsCLI(currentTime: number): Array<{element: any, track: any, mediaItem: any}> {
  ...
  async export(progressCallback?: ProgressCallback): Promise<Blob> {
    ...
    try {
      ...
      console.log(`[CLIExportEngine] 🔍 DEBUG: Keeping frames in temp directory for inspection`);
      console.log(`[CLIExportEngine] 📁 Frames location: C:\\Users\\zdhpe\\AppData\\Local\\Temp\\qcut-export\\${this.sessionId}\\frames`);
      console.log(`[CLIExportEngine] 🧪 TEST: Try this FFmpeg command manually:`);
      console.log(`cd "C:\\Users\\zdhpe\\Desktop\\vite_opencut\\OpenCut-main\\qcut\\electron\\resources" && ffmpeg.exe -y -framerate 30 -i "C:\\Users\\zdhpe\\AppData\\Local\\Temp\\qcut-export\\${this.sessionId}\\frames\\frame-%04d.png" -c:v libx264 -preset fast -crf 23 -t 5 test-output.mp4`);
  ...
  private async saveFrameToDisk(frameName: string): Promise<void> {
    ...
    try {
      ...
      if (frameName === 'frame-0000.png' || frameName === 'frame-0001.png') {
        console.log(`[CLIExportEngine] Saved ${frameName} (${base64Data.length} chars)`);
      }
      
    } catch (error) {
      console.error(`[CLIExportEngine] Failed to save frame ${frameName}:`, error);
      throw error;
    }
  }
