...
export const mockElectronAPI: ElectronAPI = {
	...
	ffmpeg: {
		...
		renderVideoFramePreview: vi.fn().mockResolvedValue({
			requestId: "preview-request",
			pngData: new Uint8Array([137, 80, 78, 71]),
			cacheHit: false,
			sourceTime: 0,
		}),
		renderVideoCompositionFramePreview: vi.fn().mockResolvedValue({
			requestId: "composition-preview-request",
			pngData: new Uint8Array([137, 80, 78, 71]),
			cacheHit: false,
			timelineTime: 0,
		}),
		cancelVideoFramePreview: vi.fn().mockResolvedValue(false),
		renderVideoPreviewProxy: vi.fn().mockResolvedValue({
			requestId: "proxy-request",
			proxyUrl: "app://video-preview-proxy/proxy.mp4",
			cacheKey: "proxy-cache-key",
			cacheHit: false,
			sourceStart: 0,
			duration: 5,
			width: 960,
		...
		extractAudioWaveform: vi.fn().mockResolvedValue({
			duration: 1,
