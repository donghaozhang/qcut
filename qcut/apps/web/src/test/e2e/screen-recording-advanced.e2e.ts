...
test.describe("Advanced Screen Recording with User Interactions", () => {
	test(
		"records screen during real editor interactions and produces valid output",
		async ({ page }) => {
			...
			const recordingToggle = page.getByTestId(
			...
			if ((await timelineRuler.count()) > 0) {
				...
				if (rulerBox) {
					...
					for (let i = 0; i < 5; i++) {
						const xPos =
							rulerBox.x + (rulerBox.width * (i + 1)) / 6;
			...
			console.log(
				`📹 Recording file size: ${(videoStats.size / 1024 / 1024).toFixed(2)} MB`
			);
			...
			console.log(
				`🖱️  Cursor telemetry points: ${sidecar.points.length}`
			);
			...
			console.log(
				`🎯 Unique cursor regions: ${uniqueXPositions.size}x${uniqueYPositions.size}`
			);

			// ── 7. Copy recording to output path ──
			const outputPath =
				"C:\\Users\\yanie\\clawd\\qcut-recording-advanced.mp4";
			await copyFile(videoPath, outputPath);
			console.log(`📂 Recording copied to: ${outputPath}`);
