...
export const useVideoSearchStore = create<VideoSearchState & VideoSearchActions>(
	(set, get) => ({
		// State
		query: "",
		results: [],
		...
		indexMedia: async (projectId, mediaId, mediaPath, mediaName, totalDuration) => {
			const api = window.electronAPI?.videoSearch;
			if (!api) return;
			...
			try {
				const result = await api.indexMedia(projectId, mediaId, mediaPath, mediaName, totalDuration);
