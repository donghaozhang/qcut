...
export function TextProperties({
	...
	return (
		<div className="p-5" data-testid="text-properties">
			<Tabs defaultValue="text">
				<TabsList className="mb-4 grid h-8 w-full grid-cols-2 rounded-sm p-0.5">
					<TabsTrigger
						value="text"
						className="min-w-0 rounded-sm px-1 text-xs"
						data-testid="text-panel-tab-text"
					>
						{t("textProperties.tab.text")}
					</TabsTrigger>
					<TabsTrigger
						value="animation"
						className="min-w-0 rounded-sm px-1 text-xs"
						data-testid="text-animation-group-toggle"
					>
						{t("textProperties.tab.animation")}
					</TabsTrigger>
				</TabsList>
				<TabsContent value="text" className="mt-0 space-y-5">
						...
						<div className="space-y-4">
							<PropertyItem direction="row">
								...
								<PropertyItemValue>
									<Select
										value={avatarImageId}
										onValueChange={setAvatarImageId}
									>
										<SelectTrigger className="h-8 text-xs">
											<SelectValue
												placeholder={t(
													"textProperties.placeholder.chooseImage"
												)}
											/>
										</SelectTrigger>
										<SelectContent>
					...
					<PropertyGroup
						title={t("textProperties.section.layout")}
						defaultExpanded
					>
						<div className="space-y-4">
							<NumberControl
