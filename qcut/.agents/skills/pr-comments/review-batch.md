...
## Git: Commit and Push After Batch

After all files are processed, commit and push all fixed source files together:

1. `git add` all modified source file(s) (not task/docs files)
2. `git commit` with a summary message (e.g., `fix: address PR #102 review comments`)
3. `git push` to the current remote branch

**Skip if no files were fixed** (all NOT_APPLICABLE or ALREADY_FIXED).
