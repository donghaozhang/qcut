...
function setupFFmpegIPC() {
  ...
  ipcMain.handle('save-frame', async (event, { sessionId, frameName, data }) => {
    try {
      ...
      if (frameName === 'frame-0000.png' || frameName === 'frame-0001.png') {
        console.log(`[FFmpeg] Saved ${frameName} (${buffer.length} bytes) to ${framePath}`);
      }
  ...
  ipcMain.handle('export-video-cli', async (event, options) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const frameFiles = fs.readdirSync(frameDir).filter(f => f.startsWith('frame-') && f.endsWith('.png'));
      ...
      const simpleArgs = [
        '-y',
        '-f', 'image2',
        '-i', path.join(frameDir, 'frame-0000.png'),
        '-c:v', 'libx264',
        '-t', '1',
        '-pix_fmt', 'yuv420p',
        outputFile
...
exit /b %ERRORLEVEL%`;
      ...
      console.log(`[FFmpeg CLI] ⚠️  Windows process spawning restricted in Electron`);
      console.log(`[FFmpeg CLI] 📁 Frames are ready at: ${frameDir}`);
      console.log(`[FFmpeg CLI] 🎬 Manual FFmpeg command:`);
      console.log(`[FFmpeg CLI] cd "${path.dirname(ffmpegPath)}" && ${path.basename(ffmpegPath)} -y -framerate 30 -i "${inputPattern}" -c:v libx264 -preset fast -crf 23 -t 5 -pix_fmt yuv420p -movflags +faststart "${outputFile}"`);
      
      // Check if user has already created the video manually
      const checkForManualVideo = () => {
        if (fs.existsSync(outputFile)) {
          const stats = fs.statSync(outputFile);
          console.log(`[FFmpeg CLI] ✅ Found manually created video! Size: ${stats.size} bytes`);
          resolve({ 
            success: true, 
            outputFile,
            method: 'manual',
